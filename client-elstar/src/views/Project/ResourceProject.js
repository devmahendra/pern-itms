...
import { Table, Input, Pagination, Select, Button, Tooltip, Card, toast, Notification } from "components/ui";
import { useSortBy, useTable, useFilters, useGlobalFilter, useAsyncDebounce, usePagination } from "react-table";
import { matchSorter } from "match-sorter";
import { HiOutlineSearch, HiDownload, HiPlusCircle, HiOutlineTrash } from "react-icons/hi";
...
const onDelete = async (ids) => {
  try {
    const deleteData = await fetch(`http://localhost:5002/resources-delete/${ids}`, {
      method: "DELETE",
    });
  ...
  return (
    <div className="flex justify-end text-lg">
      <Tooltip title="Delete">
        <span className="cursor-pointer p-2 hover:text-red-500" onClick={() => onDelete(row?.resource_id)}>
          <HiOutlineTrash />
        </span>
      </Tooltip>
    </div>
